...
static char bits[512] = {   //todo __nv
...
TASK(bit_count){//-->2, NOTE: R(9,10) || W(1,9,10)
    ...
    uint32_t tmp_seed;
    unsigned temp = 0;
    for(i = __GET(_v_iter);i<((ITER>>4)+__GET(_v_iter));i++){
...
TASK(bit_shifter){//-->8, NOTE: R(9,10) || W(7,9,10)
    ...
    for(j = __GET(_v_iter);j<((ITER>>4)+__GET(_v_iter));j++){
        tmp_seed = __GET(_v_seed);
...
void _benchmark_bc_init(void){
    if(!nvInited){
        ...
        TASK_INIT(0,    init,               0,  15);
        TASK_INIT(0,    select_func,        14,  21);
        TASK_INIT(0,    bit_count,          0,  21);
        TASK_INIT(0,    bitcount,           0,  21);
        TASK_INIT(0,    ntbl_bitcnt,        0,  21);
        TASK_INIT(0,    ntbl_bitcount,      0,  21);
        TASK_INIT(0,    BW_btbl_bitcount,   0,  21);
        TASK_INIT(0,    AR_btbl_bitcount,   0,  21);
        TASK_INIT(0,    bit_shifter,        0,  21);
        TASK_INIT(0,    end,                0,  0);
    }else{
